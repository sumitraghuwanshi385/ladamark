...
const AnimatedGrid: React.FC = () => (
  <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
    ...
    {/* Red-only glow overlays - Only red shadows/glows */}
    ...
    <section className="relative min-h-[100dvh] pt-20 pb-16 md:pt-28 flex items-center overflow-hidden bg-[var(--background-secondary)]">
      ...
      <div className="relative z-20 max-w-6xl mx-auto px-5 lg:px-8 w-full">
        <div className="flex flex-col items-center text-center">
          {/* Headline */}
          <h1 className="text-[2.55rem] leading-[1.05] md:text-6xl lg:text-[5.2rem] font-black tracking-[-2.5px] mb-8 max-w-5xl animate-fade-in-up" style={{ textWrap: 'balance' }}>
            Turn Your Catalog Into a{' '}
            <span className="animated-reddish-text bg-clip-text text-transparent bg-gradient-to-r from-rose-400 via-red-500 to-rose-600 font-black tracking-[-2px] text-[2.65rem] md:text-[5.4rem] lg:text-[5.4rem]">
              Sales Machine
            </span>
          </h1>

          {/* Subheadline - Reduced size especially on mobile */}
          <p className="text-base md:text-xl lg:text-2xl text-[var(--text-secondary)] max-w-3xl mx-auto mb-10 leading-relaxed animate-fade-in-up animation-delay-200 px-2" style={{ textWrap: 'balance' }}>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto animate-fade-in-up animation-delay-300">
            ...
            <a
              ...
              className="w-full sm:w-auto px-7 md:px-10 py-3.5 md:py-5 text-base md:text-lg font-semibold text-white border border-white/30 hover:border-white/60 backdrop-blur-md rounded-2xl transition-all duration-300 hover:bg-white/5 flex items-center justify-center"
            >
              Watch 2-min Demo
            </a>
          </div>

          {/* Bottom trust text - Smaller size */}
          <p className="mt-8 text-xs md:text-sm text-[var(--text-muted)] flex items-center gap-2 animate-fade-in-up animation-delay-500">
